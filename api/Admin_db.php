...
class Admin_db
{
    ...
    public function save_good($good_id, $name, $cover, $price, $info, $specs, $description, $images)
    {
    ...
    public function add_good($name, $cover, $price, $info, $specs, $description, $images)
    {
        $name = $this->link->real_escape_string($name);
        $cover = $this->link->real_escape_string($cover);
        $price = $this->link->real_escape_string($price);
        ...
        $images = $this->link->real_escape_string($images);
        $this->link->query(
        "INSERT INTO `goods` (`Name`, `Cover`, `Price`, `Info`, `Specs`, `Description`, `Images`, `Category-id`)
        VALUES ('$name','$cover','$price','$info','$specs','$description','$images', 1)"
        );
    }
